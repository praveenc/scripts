...
function Scrub-OWScripts
{
    ...
    Process{
        ...
        foreach($line in $lines){
            ...
            if($line -match "(ALTER|CREATE)\s+(PROCEDURE|FUNCTION|VIEW|TABLE)\s+(\S+)\s*(.*)$"){

                $sqlType = $Matches[2]
                $AfterSqlName = $Matches[4]
                $sqlName = $Matches[3] -replace "\[|\]",""
                $is_temptable = $false
                #Identify if the table is a Temp Table
                if($sqlName -match "#" -or $schemaName -match "#"){
                    $is_temptable = $true
                }
                #If SP doesn't have schema specified - then set schema to dbo
                if($sqlName -match "\.") {
                ...
                if($sqlType -eq "VIEW"){
                    ...
                    $line += "GO`r`n"
                    if([System.String]::IsNullOrEmpty($AfterSqlName)){
                        $line += "CREATE VIEW [$schemaName].[$sqlName] AS"
                    }else{
    ...
    End{
        ...
        #Read again to set encoding to UTF8
...
function Scrub-OWScriptsInFolder
{
    <#
    .Synopsis
       Scrubs All SQL Scripts in a Given Folder
    .DESCRIPTION
       Loop through all files (sql) in a given folder and calL Scrub-OWScripts
    .EXAMPLE
       Scrub-OWScriptsInFolder -FolderToScrub E:\Builds\2.3\2.3.0.150\Incremental\db-Combine
    .NOTES
        Author: Praveen Chamarthi
        Date: 22 Sep 2013
    #>
    ...
    Param(
        # Folder Path to Scrub
        [CmdletBinding()]
        [Parameter(Mandatory=$true,
                   ValueFromPipelineByPropertyName=$true,
                   Position=0,
                   HelpMessage="Complete Path to Folder Scrubbing")]
        [ValidateScript({Test-Path $_ -PathType Container})]
        [String]$FolderToScrub
    
    )

    $files = Get-ChildItem -Path $FolderToScrub -Recurse -File -Filter *.sql | Select-Object FullName

    foreach($file in $files){

        $file = $file.FullName

        Set-ItemProperty $file -Name IsReadOnly $false

        Scrub-OWScripts -FileToScrub $file
...
function Create-OWCombinePackage
{
    ...
    Process
    {
        ...
        If( (Get-ChildItem -Recurse $ScriptsFolder).Count -gt 0){
            ...
            $db_fldrs = Get-ChildItem -Path $ScriptsFolder -Directory -Recurse
            
            #Fetch all Files Under Folders
            $db_files = Get-ChildItem -Path $ScriptsFolder -File -Recurse -Filter *.sql | 
            ...
            foreach($dir in $db_fldrs){
                ...
                $parentdir = $dir.Parent.Name
                $packagerootdir = Split-Path $ScriptsFolder -Leaf
                $foldername = $dir.Name

                if($packagerootdir -eq $parentdir){
                
                    $rel_folder = ""
                    Write "----- Adding folder $foldername to $PackageName.cpa " -Verbose

                }else{
                    
                    $rel_folder = $parentdir
                    Write "----- Adding folder $foldername --> $rel_folder to $PackageName.cpa " -Verbose
                
                }

                
                $cp_args = "/AddFolder /p:""$ScriptsFolder\$PackageName"" /n:""$foldername"" /r:""$rel_folder"" /v"
                ...
                try{
                    ...
                    Write "Error Adding Folder $foldername to $PackageName.cpa" -Verbose
            ...
            foreach($file in $db_files){
                ...
                $scriptnameandpath = $file.FullName
                $dirname = $file.Directory.Name
                $filename = $file.Name
                $packagerootdir = Split-Path $ScriptsFolder -Leaf

                if($packagerootdir -eq $dirname){
                
                    $foldername = ""

                }else{
                    
                    $tmpRoot = $ScriptsFolder -replace "\\","\\"
                    $dirname = ($file.Directory.FullName) -replace "$tmpRoot\\",""
                    $foldername = "$dirname"

                }

                Write "----- Adding file $filename to $foldername" -Verbose
                ...
                try{
                    ...
                    Write "Error Adding script $scriptnameandpath to $PackageName.cpa" -Verbose
                    Write $_.Exception.Message
